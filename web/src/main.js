...
(async () => {
	const model = await app("#app", {
		...
		async mounted() {
			...
			// Double-check webkit context before loading debug content to prevent flashing
			if (this.allowDebug) {
				let exampleContent =
					document.getElementById("example-md")?.textContent;
				if (!exampleContent) {
					exampleContent = await fetch("example.md").then((res) =>
						res.text()
					);
				}
